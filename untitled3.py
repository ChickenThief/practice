...
output=['name', 'of', 'a', 'thing', 'that', 'looks', 'like', 'code', 'but', 'doesn\'t', 'do ']
darkness="words"
def change():
        answer=darkness
        answer+=" Names of Oceans"
        return(answer) 
        ScrollApp.run
...
class ScrollApp(App):
    ...
    def build(self):
        ...
        closebutton.bind(on_press=popup.open)
        ...
        self.txt1 = TextInput(size_hint =  (1,None), height = 30, multiline= False,  text= '')     
        layout2.add_widget(self.txt1)
        for index, i in enumerate(output):
            title= (str(index) +". "+ str(i))
            btn = Button(text=str(i), size_hint_y=None, height=30, valign='middle', font_size=12)
